package com.ouat.customerService.client;
...
public class EmailSMSServiceClient {
	...
	public boolean sendOTPOnMobile(OTPRequestForEmailSMSService request) {
		boolean flag = false;
		HttpHeaders headers = new HttpHeaders();
		headers.setAccept(Arrays.asList(MediaType.APPLICATION_JSON));
		HttpEntity<OTPRequestForEmailSMSService> entity = new HttpEntity<>(request, headers);
		try {
			DemandStoreAPIResponse response = restTemplate
					.exchange(emailSMSServiceURL + "/sms/otp", HttpMethod.POST, entity, DemandStoreAPIResponse.class)
					.getBody();
			if (null != response && null != response.getCode() && "200".equals(response.getCode())) {
				flag = true;
				return flag;
